...
void send_request(RawSockStream&, const std::string&);
bool recv_reply(RawSockStream&, ECHO_REPLY&);
...
#define CLOCKS_DIV 1.0/(CLOCKS_PER_SEC+0.0)
...
void ping(const std::string& host) {
  RawSockStream ping_socket(FreeSockets::proto_ICMP);
  if(!ping_socket) {
...
void send_request(RawSockStream& sock, const std::string& host) {
  ...
  echoReq.icmpHdr.Checksum = in_cksum((unsigned short*)&echoReq,sizeof(ECHO_REQUEST));

  const sockaddr_storage & sst = sock.getOutpeer();

  char hbuf[NI_MAXHOST];

  if (::getnameinfo((const sockaddr*)&sst, sock.getOutpeerSize(), hbuf, sizeof(hbuf), 0, 0, NI_NUMERICHOST) == 0) {
...
bool recv_reply(RawSockStream& sock, ECHO_REPLY& reply) {
  sock.setTimeout(3,0);
...
void print_statistics(const ECHO_REPLY& reply) {
  ...
  if(iter == statistics.end())
    iter = statistics.insert(
      std::pair<std::string, std::pair<unsigned, unsigned> >
      (replier,std::pair<unsigned, unsigned>(0,0))).first;
...
bool is_broadcast_address() { return true; }

...
static void byteReverse (nat1 *buf, unsigned longs){
  uint32 t;
  do {
...
static void MD5Update(MD5Context *pCtx, const nat1 *buf, unsigned int len){
  struct Context *ctx = (struct Context *)pCtx;
  uint32 t;
...
static void MD5Final(nat1 digest[16], MD5Context *pCtx){
  struct Context *ctx = (struct Context *)pCtx;
  unsigned count;
  nat1 *p;
...
** "nat1 digest[16]" in the calling function.  The MD5
** digest is stored in the first 16 bytes.  zBuf should
** be "char zBuf[33]".
*/
static void DigestToBase16(nat1 *digest, char *zBuf){
...
static int md5_cmd(void* cd, Tcl_Interp *interp, int argc, const char **argv){
  ...
  MD5Init(&ctx);
  MD5Update(&ctx, (nat1*)argv[1], (unsigned)strlen(argv[1]));
  MD5Final(digest, &ctx);
...
static int md5file_cmd(void*cd, Tcl_Interp*interp, int argc, const char **argv){
  ...
  for(;;){
    ...
    if( n<=0 ) break;
    MD5Update(&ctx, (nat1*)zBuf, (unsigned)n);
  }
  fclose(in);
  MD5Final(digest, &ctx);
  DigestToBase16(digest, interp->result);
...
static void md5step(sqlite3_context *context, int argc, sqlite3_value **argv) {
  if( argc >= 1 ) {
    ...
    for(i=0; i<argc; i++) {
      if( argv[i] ) {
	MD5Update(p, (nat1*)argv[i], strlen((const char*)argv[i]));
      }
    }
  }
}
